...
TLS_APPL_STATE *tls_server_init(const TLS_SERVER_INIT_PROPS *props)
{
	 ...
	 * and retired key validation lifetime of session tickets keys.
	 * This way a session always lasts longer than the server's ability
	 * to decrypt its session ticket.  Otherwise, a bug in OpenSSL may
	 * fail to re-issue tickets when sessions decrypt, but are expired.
	 */
