...
/*	#include "smtp_addr.h"
/*
/*	DNS_RR *smtp_domain_addr(name, iter, misc_flags, why, found_myself)
/*	char	*name;
/*	SMTP_ITERATOR *iter;
/*	int	misc_flags;
/*	DSN_BUF	*why;
...
/*	and dnssec validation status of the MX RRset are recorded
/*	in iter->mx, which the caller must free with dns_rr_free().
/*
...
DNS_RR *smtp_domain_addr(char *name, SMTP_ITERATOR *iter, int misc_flags,
...
#define IMPOSSIBLE_PREFERENCE	(~0)
	...
	iter->mx = dns_rr_copy(mx_names);	/* One RR is enough */
